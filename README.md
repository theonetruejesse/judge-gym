...
- Late Temporal callbacks that arrive after reset cleanup has removed a run/window/attempt target now degrade to safe worker-side no-ops: snapshot projection emits a `process_projection_skipped_missing_target` breadcrumb on the known process trace, and late attempt-finish callbacks log a warning instead of crashing the Convex mutation.
...
- Score-stage batch preamble now also stages `recordLlmAttemptStart` with bounded concurrency and emits `batch_preamble:record_attempt_starts` heartbeats, which prevents large 500-item score batches from saturating Convex writes before batch registration begins.
