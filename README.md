...
Bundled provider ceilings now include Anthropic Claude Sonnet 4 standard tiers `tier_1` through `tier_4`. OpenAI now defaults to `tier_2` with conservative bundled model ceilings, and batch execution policy remains separately configurable in `engine-settings` and is capped by provider/model ceilings at runtime.
