...
- Synthetic fault injection is temporarily enabled only for the reduced-load live chaos pass on a hardcoded subset of V3 experiment ids, injecting retryable parse faults into the first two attempts for a deterministic slice of targets; it should be reverted immediately after the chaos run. Historical matrix reports remain under `apps/engine-convex/docs/`.
