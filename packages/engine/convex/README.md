...
- Experiment status reporting now follows the latest run for operator surfaces like `listExperiments` / `getExperimentSummary`, so a historical failed run does not keep an experiment labeled `error` after a later successful rerun.
