...
- Completed/error run wipes now page through `scores`, `score_critics`, `sample_score_targets`, and `sample_score_target_items` by index instead of preloading the full run tree into one delete plan, which keeps heavy score-stage resets under the Convex bytes-read ceiling.
- V3 reset now retains historical `llm_requests` rows as audit history instead of deleting them during a wipe; the reset removes live run/orchestration/artifact state and zeroes experiment totals, which makes fresh relaunches reliable without timing out on heavyweight prompt/output payload rows.
