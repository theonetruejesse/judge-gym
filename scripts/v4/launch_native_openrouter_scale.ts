...
type Args = {
  ...
  runTimeoutMs: number;
  pollMs: number;
  providers: NativeOpenRouterProvider[];
};
...
function parseArgs(argv: string[]): Args {
  const args: Args = {
    ...
    pollMs: 5_000,
    providers: [...NATIVE_OPENROUTER_PROVIDERS],
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--providers" && next) {
      ...
      args.providers = NATIVE_OPENROUTER_PROVIDERS.filter((provider) => requested.includes(provider));
      index += 1;
      continue;
    }
  }
...
async function main() {
  ...
  const launches = args.providers.map((provider) => ({
    provider,
    manifest_path: path.join(OPENROUTER_SCALE_ROOT, `${nativeModelTagSuffix(provider)}.json`),
  }));
  const plan = {
    live: args.live,
  ...
  const summaryPath = path.join(
    OPENROUTER_SCALE_ROOT,
    `live_openrouter_launch_${isoTimestampTag()}.json`,
  );
  const summary = {
    ...plan,
    outputs,
