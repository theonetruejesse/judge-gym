...
const OPENAI_SCALE_INDEX_PATH = path.join(OPENAI_SCALE_ROOT, "index.json");
const FULL_VOLUME_TARGET_COUNT = 30;
...
function providerTitle(model: NativeOpenAIProvider) {
  switch (model) {
    case "gpt-4.1":
      return "GPT-4.1";
    case "gpt-5.2":
...
function buildBlueprints(model: NativeOpenAIProvider): NativeConceptualManifest["experiment_blueprints"] {
...
async function main() {
  ...
  await ensureDir(OPENAI_SCALE_ROOT);

  const manifests = [];
  for (const model of NATIVE_OPENAI_PROVIDERS) {
    const manifest: NativeConceptualManifest = {
      ...baseManifest,
      manifest_tag: `v4_native_concepts_${nativeModelTagSuffix(model)}_fullscale_v1`,
      title: `V4 native conceptual ${providerTitle(model)} promoted matrix`,
      provider: model,
      transform: {
    ...
    const manifestPath = path.join(OPENAI_SCALE_ROOT, `${nativeModelTagSuffix(model)}.json`);
  ...
  const index = {
    root: OPENAI_SCALE_ROOT,
    target_count: FULL_VOLUME_TARGET_COUNT,
    evidence_set_tag: baseManifest.curation.curated_evidence_set_tag,
    promoted_lanes: PROMOTED_NATIVE_LANES,
    manifests,
  };
  await writeJson(OPENAI_SCALE_INDEX_PATH, index);
