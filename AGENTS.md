...
Open-source LLM-as-Judge design-space engine. Turborepo monorepo with Bun (`packages/engine-convex`, `packages/engine-temporal`, `packages/engine-settings`) and uv (`packages/analysis`).

## Structure

- `packages/engine-convex/convex/` — Convex backend
- `packages/engine-temporal/` — Temporal worker runtime
- `packages/engine-settings/` — shared runtime-agnostic config/constants
- `packages/analysis/` — Python analysis
- `_campaigns/` — campaign state and machine-readable launch/reset manifests
- `skills/` — repo-versioned Codex skills

## Convex Code Style

- Use zod helpers from `packages/engine-convex/convex/utils.ts` (`zMutation`, `zQuery`, `zInternalAction`, etc.).
...
- After any Convex code or schema change, run the full validation routine:
  ...
  2. `cd packages/engine-convex && npx convex codegen`, then `bun run typecheck` from repo root.
...
1. Identify the deployment with `mcp__convex__status` using `packages/engine-convex`.
