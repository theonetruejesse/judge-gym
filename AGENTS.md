...
Open-source LLM-as-Judge design-space engine. Turborepo monorepo with Bun (`packages/engine`) and uv (`packages/analysis`).

## Structure

- `packages/engine/convex/` — Convex backend
...
- Use zod helpers from `packages/engine/convex/utils.ts` (`zMutation`, `zQuery`, `zInternalAction`, etc.).
...
- After any Convex code or schema change, run the full validation routine:
  ...
  2. `cd packages/engine && npx convex codegen`, then `bun run typecheck` from repo root.
...
1. Identify the deployment with `mcp__convex__status` using `packages/engine`.
