...
- `packages/lab:getRunSummary` now includes persisted `completed_count`, per-stage run counters, and `pause_after`, which is the sample-level monitoring surface backing the lab run table.
